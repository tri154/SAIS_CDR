...
class Transformer(nn.Module):
    ...
    def forward(self, batch_token_seqs, batch_token_masks, batch_token_types):
        print(batch_token_seqs.shape) # 1158
        ...
        real_num_tokens = batch_token_masks.sum(1).int().tolist() # num_doc, lenght_of_that_doc
        for doc_id, real_num_token in enumerate(real_num_tokens): # for each docs.
            if real_num_token <= self.max_num_tokens:
                ...
                new_token_type1 = torch.cat([batch_token_types[doc_id, :self.max_num_tokens - self.end_token_len],
                                            batch_token_types[doc_id, self.max_num_tokens - self.end_token_len - 1].repeat(self.end_token_len)], dim=-1)
                ...
                new_token_type2 = torch.cat([batch_token_types[doc_id, real_num_token - self.max_num_tokens + self.start_token_len].repeat(self.start_token_len),
                                            batch_token_types[doc_id, real_num_token - self.max_num_tokens + self.start_token_len : real_num_token]], dim=-1)
        ...
        for (new_token_seq, real_num_token) in zip(new_token_segs, real_num_tokens):
            if new_token_seq == 1:
                ...
                new_token_mask = new_token_mask1 + new_token_mask2 + self.cfg.small_positive - self.cfg.small_negative
                print(torch.where((new_token_mask == 0))) # error HERE
                new_token_emb = (new_token_emb1 + new_token_emb2) / new_token_mask.unsqueeze(-1)
                print(new_token_emb[511:647]) # nan
        ...
        batch_token_embs = torch.stack(new_token_embs, dim=0)
        batch_token_atts = torch.stack(new_token_atts, dim=0)
