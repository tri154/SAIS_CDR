...
class DocREModel(nn.Module):
    ...
    def forward(self, context_output, attention_mask, entity_pos, hts, labels=None):
        attention_map = self.segmentation_net(context_output.permute(0, 2, 1)).permute(0, 2, 1)

        bs, slen, hdim = context_output.size()
        pred = []
        loss = 0

        for i in range(bs):
            output = context_output[i]
            attn_map = attention_map[i]
            entities = []
            for e in entity_pos[i]:
                entity_embeds = output[e[0]:e[1] + 1]
                weights = attn_map[e[0]:e[1] + 1].mean(dim=1).unsqueeze(-1)  # [len, 1]
                entity = (weights * entity_embeds).sum(dim=0) / weights.sum()
                entities.append(entity)

            h_t_pairs = []
            for h, t in hts[i]:
                h_t = torch.cat([entities[h], entities[t]], dim=-1)
                h_t_pairs.append(h_t)

            h_t_pairs = torch.stack(h_t_pairs)  # [num_pairs, hidden*2]
            pair_out = self.bilinear(h_t_pairs)  # [num_pairs, num_relations]
            pred.append(pair_out)

            if labels is not None:
                label_tensor = torch.tensor(labels[i], dtype=torch.float, device=pair_out.device)
                loss += F.binary_cross_entropy_with_logits(pair_out, label_tensor)
