...
class Transformer(nn.Module):
    ...
    def __init__(self, info, transformer, tokenizer):
        ...
        self.start_token_ids = torch.Tensor([tokenizer.cls_token_id]).to(info.DEVICE_GPU)
        self.end_token_ids = torch.Tensor([tokenizer.sep_token_id]).to(info.DEVICE_GPU)
...
class Model(nn.Module):
    ...
    def __init__(self, args, info, transformer, rtype_embeddings, tokenizer):
        ...
        self.transformer_module = Transformer(info, transformer, tokenizer)
    ...
    def get_CR_reps(self, batch_token_embs, batch_mention_tids, batch_num_mentions_per_doc):
        ...
        for doc_token_embs, doc_mention_tids in zip(batch_token_embs, batch_mention_tids):
            ...
            doc_mention_reps = torch.einsum('mbi,nbj->mnbij', doc_mention_embs, doc_mention_embs ).flatten(2, 4).flatten(0, 1)
    ...
    def get_PER_reps(self, batch_epair_contexts, batch_num_epairs_per_doc, batch_sent_embs):
        ...
        batch_PER_reps = [torch.einsum('ebi,sbj->esbij', doc_epair_contexts, doc_sent_embs) for doc_epair_contexts, doc_sent_embs in zip(batch_epair_contexts, batch_sent_embs)] # doc: num_epair * num_sent * num_block * size_block * size_block
    ...
    def get_FER_reps(self, batch_epair_embs, batch_epair_contexts, batch_epair_relations, batch_num_epairs_per_doc, batch_sent_embs):
        ...
        for doc_triplet_reps, doc_sent_embs in zip(batch_triplet_reps, batch_sent_embs):
            ...
            doc_FER_reps = torch.einsum('tbi,sbj->tsbij', doc_triplet_reps, doc_sent_embs).flatten(2,4).flatten(0,1)
